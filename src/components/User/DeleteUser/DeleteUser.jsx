...
import { useDeleteUserByAuthorityMutation } from "../../../redux/features/auth/authApi";
...
const DeleteEmployee = ({ id, isLoading }) => {
  ...
  const [deleteUserByAuthority, { isLoading: deleteLoading }] =
    useDeleteUserByAuthorityMutation();
  const handleRemoveConfirm = async () => {
    if (selectedId) {
      try {
        const res = await deleteUserByAuthority(selectedId).unwrap();
        if (res?.success) {
          setIsOpen(false);
          setSelectedId(null);
  ...
  return (
    <>
      {user?.user_id !== id && (
        <button
          disabled={deleteLoading || isLoading || user?.user_id == id}
